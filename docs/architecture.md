...
- `BrowserBridge`：与 Chromium Native Messaging helper 通信；发送结构化命令。
...
WebExtension 只接受结构化命令：`prepareX` 与 `postX(text)`。它不执行插件提供的任意 JavaScript。Chromium 通过 Native Messaging helper 与 App 的本地 Unix socket 桥接；Safari 使用同一份 X DOM core，并由打包的 Safari Web Extension handler 承接 native message。
